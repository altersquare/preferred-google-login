...
function addDomainEmailPair(container, domain = "", email = "",saveButton) {
    ...
    domainInput.addEventListener("input",  () => {
		...
		const domains = Array.from(container.querySelectorAll(".domain-input"))
			.map(input => input.value.trim());

		const isDuplicate = domains.filter(d => d === trimmedDomain).length > 1;
...
async function handleSaveClick() {
    ...
    const domainEmailContainers = document.querySelectorAll(".domain-email-container");
    ...
    domainEmailContainers.forEach(container => {
        ...
        const emailInput = container.querySelector(".input-email-container input");
        ...
        if (domainEmails[domain]) {
            isValid = false;
            container.querySelector(".error-message:first-of-type").style.display = "block";
        } else {
        ...
        if (!validateEmail(email)) {
            isValid = false;
            container.querySelector(".error-message:last-of-type").style.display = "block";
        }
    });
